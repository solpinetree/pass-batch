package com.sol.passbatch.job.notification;

import com.sol.passbatch.repository.booking.BookingEntity;
import com.sol.passbatch.repository.booking.BookingStatus;
import com.sol.passbatch.repository.notification.NotificationEntity;
import com.sol.passbatch.repository.notification.NotificationEvent;
import com.sol.passbatch.repository.notification.NotificationModelMapper;
...
public class SendNotificationBeforeClassJobConfig {
     ...
     */
    // -> 이번에 조회하는 데이터들은 조회한 후에 업데이트가 일어나지 않으므로
    // 커서를 사용할 필요가 없다. 수정이 일어나는 작업에 paging을 쓸 경우 누락이 발생할 수 있다.
    @Bean
    ...
    public Step sendNotificationStep(JobRepository jobRepository, PlatformTransactionManager platformTransactionManager) {
        return new StepBuilder("sendNotificationStep", jobRepository)
                ...
                .taskExecutor(new SimpleAsyncTaskExecutor())  // 가장 간단한 멀티쓰레드 TaskExecutor 를 선언
                .build();
    }
